...
function resolveDataDir() {
  ...
  try {
    ...
    const tmp = path.join(os.tmpdir(), 'arc_data');
    if (!fs.existsSync(tmp)) {
      try { fs.mkdirSync(tmp, { recursive: true }); } catch (err) {}
    }
    return tmp;
  }
}
...
module.exports = function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    if (wallet && !/^0x[a-fA-F0-9]{40}$/i.test(wallet)) {
      res.statusCode = 400;
      return res.end(JSON.stringify({ error: 'Invalid EVM wallet format' }));
    }

    const sanitized = { name, wallet, score, round, date, _ts: ts, _n: nonce, _sig: sig };
